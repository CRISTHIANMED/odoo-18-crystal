            ...
            if provider.graph_api_authenticated:
                ...
                if data_value_obj.get('messages'):
                    for mes in data_value_obj.get('messages'):
                        existing_mail_message = request.env['mail.message'].sudo().search(
                            [('wa_message_id', '=', mes.get('id'))])
                        existing_whatsapp_history = request.env['whatsapp.history'].sudo().search(
                            [('message_id', '=', mes.get('id'))])
                        if existing_whatsapp_history or existing_mail_message:
                            return wa_dict
