...
class Provider(models.Model):
    ...
    def extend_meta_temporary_access_token(self):
        if not all([self.graph_api_url, self.graph_api_app_id,
                    self.graph_api_app_secret, self.graph_api_token]):
            raise ValidationError("Missing required API credentials")

        url = f'{self.graph_api_url}oauth/access_token'
        params = {
            'grant_type': 'fb_exchange_token',
            'client_id': self.graph_api_app_id,
            'client_secret': self.graph_api_app_secret,
            'fb_exchange_token': self.graph_api_token
        }
        headers = {'Content-Type': 'application/json'}
        ...
        try:
            ...
            if not data.get('access_token') or not data.get('expires_in'):
                raise ValidationError("Invalid response from API: missing token or expiry")

            self.graph_api_token = data['access_token']
            self.access_token_expiry_date = fields.Datetime.now() + timedelta(seconds=data['expires_in'])
            ...
            raise ValidationError(f"Invalid JSON response from API: {str(e)}")
        except Exception as e:
